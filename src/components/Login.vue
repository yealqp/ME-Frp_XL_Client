      ...
      <n-card class="login-card" :bordered="true">
        ...
        <n-form ref="formRef" :model="loginForm" :rules="rules" @submit.prevent="handleLogin">
          ...
          <n-button type="primary" size="large" block :loading="isLogging"
            :disabled="isTokenMode ? !loginForm.userToken : (!loginForm.username || !loginForm.password || !loginForm.captchaToken)" 
...
async function openTokenPage() {
  try {
    ...
    message.error('无法打开链接，请手动访问 https://www.mefrp.com/dashboard/profile')
  }
}
...
async function handleTokenLogin() {
  try {
    console.log('开始Token登录，Token长度:', loginForm.value.userToken?.length || 0)
    ...
    // 首先加载当前配置
    const currentConfig = await invoke('load_unified_config') as UnifiedConfig
    ...
    // 保存更新后的配置
    const saveResult = await invoke('save_unified_config', { config: updatedConfig })
    ...
    if (!verifyConfig.userToken || verifyConfig.userToken !== loginForm.value.userToken.trim()) {
    ...
    console.log('最新配置加载成功，frpToken 长度:', latestConfig.frpToken?.length || 0)
