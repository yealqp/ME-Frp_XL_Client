...
export function useCap(config: CapConfig) {
  ...
  const handleSolve = (event: CapSolveEvent) => {
    if (isMounted.value) {
      ...
      if (captchaToken) {
        ...
        console.log(
          `[${timestamp}] Cap.js verification completed successfully:`,
          captchaToken,
        );
      }
    }
  ...
  const handleProgress = (event: any) => {
    if (isMounted.value) {
      ...
      if (progress >= 10 && progress <= 15) {
        console.warn(
          `[${timestamp}] Cap.js verification potentially stuck at ${progress}% - monitoring for timeout`,
        );
        ...
        setTimeout(() => {
          if (isMounted.value && !isVerified.value && !error.value) {
            ...
            if (currentProgress >= 10 && currentProgress <= 15) {
              console.error(
                `[${new Date().toISOString()}] Cap.js stuck at ${currentProgress}% for too long, triggering retry`,
              );
              // 触发重试
              if (initAttempts.value < maxInitAttempts.value) {
                setTimeout(() => {
                  if (isMounted.value && capWidget.value) {
                    console.log(
                      "Attempting to reset widget due to progress stuck",
                    );
                    reset();
                  }
                }, 2000);
  ...
  const initCap = async (containerId: string) => {
    const timestamp = new Date().toISOString();
    console.log(
      `[${timestamp}] Cap.js initialization started (attempt ${initAttempts.value + 1}/${maxInitAttempts.value})`,
      {
        containerId,
        config: {
          apiEndpoint: config.apiEndpoint,
    ...
    try {
      if (!isMounted.value) {
        console.warn(
          `[${timestamp}] Component unmounted, aborting initialization`,
        );
        return;
      }
      ...
      while (retryCount < maxRetries && isMounted.value) {
        ...
        if (retryCount < maxRetries - 1) {
          console.log(
            `Container ${containerId} not ready, retrying... (${retryCount + 1}/${maxRetries})`,
          );
      ...
      if (!container || !document.contains(container)) {
        if (isMounted.value) {
          throw new Error(
            `Container ${containerId} not found after ${maxRetries} attempts`,
          );
        }
        return;
      }
      ...
      // 创建 cap-widget 元素
      const capWidgetElement = document.createElement(
        "cap-widget",
      ) as CapWidget;
      capWidgetElement.id = `cap-${Math.random().toString(36).substr(2, 9)}`;
      capWidgetElement.setAttribute(
      ...
      if (config.theme) {
        ...
        if (theme.primaryColor)
          capWidgetElement.style.setProperty(
            "--cap-primary-color",
            theme.primaryColor,
          );
      ...
      verificationTimeout.value = window.setTimeout(() => {
        if (isMounted.value && !isVerified.value) {
          ...
          console.error(
            `[${timeoutTimestamp}] Cap.js verification timeout after ${VERIFICATION_TIMEOUT_MS}ms`,
          );

          if (initAttempts.value < maxInitAttempts.value) {
            console.log(
              `[${timeoutTimestamp}] Retrying initialization (${initAttempts.value}/${maxInitAttempts.value})`,
            );
            // 重试初始化
            setTimeout(() => {
              if (isMounted.value) {
      ...
      if (isMounted.value) {
        const errorMessage =
          err instanceof Error ? err.message : "Cap.js 初始化失败";
        error.value = errorMessage;
        isLoading.value = false;
      }
    }
